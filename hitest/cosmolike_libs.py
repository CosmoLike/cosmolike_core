...
initcosmo_runmode=lib.init_cosmo_runmode
initcosmo_runmode.argtypes=[ctypes.c_char_p]
...
horndeski_sigma = lib.horndeski_sigma
horndeski_sigma.argtypes = [ctypes.c_double,]
horndeski_sigma.restype = ctypes.c_double

horndeski_mu = lib.horndeski_mu
horndeski_mu.argtypes = [ctypes.c_double,]
horndeski_mu.restype = ctypes.c_double

horndeski_M2 = lib.horndeski_M2
horndeski_M2.argtypes = [ctypes.c_double,]
horndeski_M2.restype = ctypes.c_double

set_horndeski_params = lib.set_horndeski_params
set_horndeski_params.argtypes = [ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double]
set_horndeski_params.restype = None

_horndeski_function = lib.horndeski_function
_horndeski_function.argtypes = [ctypes.c_double, ctypes.c_char_p, ctypes.POINTER(ctypes.c_int)]
_horndeski_function.restype = ctypes.c_double

def horndeski_function(a, funcname):
    status = ctypes.c_int(0)
    f = _horndeski_function(a, funcname, ctypes.byref(status))
...
class LikelihoodFunctionWrapper(object):
    ...
    def __call__(self, x):
        ...
        #inp.print_struct()
        #print
        print "... Hit likelihood..."
        t0 = time.time()
        like = lib.log_like_wrapper(icp, inp)
        print "\t%3.3f sec (logL = %3.3e)" % (time.time() - t0, like)
        return like
...
lib.compute_data_vector_wrapper.argtypes = [ctypes.c_char_p, InputCosmologyParams, InputNuisanceParams]
lib.compute_data_vector_wrapper.restype = None
compute_data_vector = lib.compute_data_vector_wrapper

def sample_cosmology_only(MG = False, horndeski = None):
    
    # Add all basic cosmological parameters into the list
    ...
    # Add mu-Sigma parameters if requested
    if MG:
        varied_parameters += ['MGSigma', 'MGmu']
    
    # Add Horndeski parameters if requested
    valid_horndeski = ['alpha_k', 'alpha_b', 'alpha_m', 'alpha_t', 'alpha_M2']
    if horndeski is not None:
        # Loop over specified parameters and perform sanity check on each one
        for h in horndeski:
            if h in valid_horndeski:
                varied_parameters.append(h)
            else:
                print "Parameter '%s' not recognized in sample_cosmology_only(). Ignoring." % h
    return varied_parameters

def sample_cosmology_shear_nuisance(tomo_N_shear, MG = False, horndeski = None):
    varied_parameters = sample_cosmology_only(MG, horndeski)
...
def sample_cosmology_2pt_nuisance(tomo_N_shear, tomo_N_lens, MG = False, horndeski = None):
    varied_parameters = sample_cosmology_only(MG, horndeski)
    varied_parameters += ['shear_m_%d'%i for i in xrange(tomo_N_shear)]
    varied_parameters += ['source_z_bias_%d'%i for i in xrange(tomo_N_shear)]
...
def sample_cosmology_2pt_nuisance_IA_marg(tomo_N_shear,tomo_N_lens,MG = False, horndeski = None):
    varied_parameters = sample_cosmology_only(MG, horndeksi)
    varied_parameters += ['shear_m_%d'%i for i in xrange(tomo_N_shear)]
    varied_parameters += ['source_z_bias_%d'%i for i in xrange(tomo_N_shear)]
    varied_parameters += ['lens_z_bias_%d'%i for i in xrange(tomo_N_lens)]
    varied_parameters += ['bias_%d'%i for i in xrange(tomo_N_lens)]
...
def sample_cosmology_2pt_cluster_nuisance(tomo_N_shear,tomo_N_lens, MG = False, horndeski = None):
    if MG:
