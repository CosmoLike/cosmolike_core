...
double int_for_Psi_L(double a, void *params){
  static double g0 = 0.;
  if (g0 ==0){g0 =1./growfac(1.);}
  double *ar = (double *) params;
  int l = (int) ar[0];
  ...
  return growfac(a)*g0*res*W_kappa(a, fK, ar[2])*sqrt((l+2.)*(l+1.)*l*(l-1.))/x/x * dchi_da(a);
}
...
double Psi_cl_withMag(double k, int l,int ni){
  ...
  return int_gsl_integrate_low_precision(int_for_Psi_cl_withMag,(void*)ar,tomo.clustering_zmin[ni],tomo.clustering_zmax[ni],NULL,100);
}

double Psi_Mag(double k, int l,int ni){
  double ar[3] = {(double)l,k,(double) ni};
  return int_gsl_integrate_low_precision(int_for_Psi_Mag,(void*)ar,tomo.clustering_zmin[ni],tomo.clustering_zmax[ni],NULL,100);
}
...
double Psi_L(double k, int l,int ni){
  ...
  //   exit(0);
  return int_gsl_integrate_low_precision(int_for_Psi_L,(void*)ar,amin_source(ni),amax_source(ni),NULL,100);
}
...
double C_gl_non_Limber(int l, int ni, int nj){ //includes RSD too!
  int ar[3] ={l,ni,nj};
  double res;
  if (2*l+5 > GSL_SF_DOUBLEFACT_NMAX){
  ...
  if (recompute_ggl(C,G,N,ni)){
    ...
    for (nz = 0; nz <tomo.ggl_Npowerspectra; nz ++){
      ...
      while (fabs(dev) > tolerance){
        ...
        dev = Cl[L]/C_gl_tomo_nointerp((double)L,ZL(nz),ZS(nz))-1.;
       printf("nz,L=%d %d %e %e\n",nz,L,Cl[L],dev);
        fprintf(OUT, "%d %lg %lg %lg\n", L, Cl[L], C_gl_tomo_nointerp(1.*L,ZL(nz),ZS(nz)), C_gl_lin_nointerp(1.*L,ZL(nz),ZS(nz)));
        L = L+1;
      }
      fclose(OUT);
      exit(0);
      // printf("switching to Limber calculation at l = %d\n",L);
      for (l = 1; l < LMAX; l++){
        Cl[l]=C_gl_tomo((double)l,ZL(nz),ZS(nz));
